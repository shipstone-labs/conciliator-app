...
async function runSubscriptionTest() {
  ...
  try {
    ...
    await page.screenshot({
      path: path.join(screenshotsDir, 'testid-02-before-learn-click.png'),
    })
    ...
    const keyFeatures = page.locator(
    ...
    const protectionProcess = page.locator(
    ...
    await page.screenshot({
      path: path.join(screenshotsDir, 'testid-03-how-it-works-page.png'),
    })
    ...
    await page.screenshot({
      path: path.join(screenshotsDir, 'testid-04-before-assessment-click.png'),
    })
    ...
    const questionType = page.locator(
      '[data-testid="assessment-question-type"]'
    )
    await questionType.waitFor({ state: 'visible', timeout: 10000 })
    console.log('Found first question (type)')

    await page.screenshot({
      path: path.join(screenshotsDir, 'testid-05-assessment-page.png'),
    })
    ...
    await page.screenshot({
      path: path.join(screenshotsDir, 'testid-error.png'),
    })
