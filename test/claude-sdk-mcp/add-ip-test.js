...
async function runAddIPTest() {
  ...
  const { mdPath, ideaJson } = saveIdea()
  ...
  console.log(
    '   File contains: Full trade secret document with restricted information'
  )
  ...
  const testUrl =
    ...
    'https://pr-137---conciliator-55rclsk2qa-uc.a.run.app'
  ...
  try {
    ...
    if (await encryptButton.isVisible()) {
      ...
      await page.waitForSelector(
        '[data-testid="add-encrypt-button"][disabled]',
        { timeout: 60000 }
      )
    ...
    const setTermsButton = await page.locator(
    ...
    if (await setTermsButton.isVisible()) {
      ...
      await page.click(
        `[data-testid="ip-business-model-option-${TEST_DATA.businessModel}"]`
      )
    ...
    if (await aiCheckbox.isVisible()) {
      ...
      console.log(
        `   Current AI agent setting: ${isChecked ? 'Enabled' : 'Disabled'}`
      )
      if (!isChecked) {
    ...
    if (isDisabled) {
      ...
      const hasTitle = await page
        .locator('[data-testid="idea-title-input"]')
        .inputValue()
      const hasDescription = await page
        .locator('[data-testid="idea-description-input"]')
        .inputValue()
    ...
    try {
      ...
      const loadingElement = await page
        .locator('.animate-spin, svg.lucide-loader-2')
        .first()
