  ...
  if [ ! -d "submods/js-sdk/packages/contracts" ] && [[ " ${REQUIRED_PACKAGES[@]} " =~ " @lit-protocol/contracts " ]]; then
    ...
    if [ -d "submods/js-sdk/packages/contracts-sdk" ]; then
      ...
      if [ -n "$pkg_file" ]; then
        ...
        if [ -f "$ROOT_DIR/dist-packages/package-map.file.json" ] && [ ! -L "$ROOT_DIR/dist-packages/package-map.file.json" ]; then
          # If it's a real file, use jq to update it
          jq --arg pkg "@lit-protocol/contracts" --arg file "file:./dist-packages/$target_file" '.[$pkg] = $file' "$ROOT_DIR/dist-packages/package-map.file.json" > "$temp_map"
        else
          # If it doesn't exist or is a symlink, start fresh
          if [ -L "$ROOT_DIR/dist-packages/package-map.file.json" ]; then
            rm -f "$ROOT_DIR/dist-packages/package-map.file.json"
          fi
          echo "{\"@lit-protocol/contracts\": \"file:./dist-packages/$target_file\"}" > "$temp_map"
        fi
        ...
        if [ -L "$ROOT_DIR/dist-packages/package-map.file.json" ]; then
          echo "⚠️ ERROR: package-map.file.json is still a symlink. Creating it directly..."
          rm -f "$ROOT_DIR/dist-packages/package-map.file.json"
          echo "{\"@lit-protocol/contracts\": \"file:./dist-packages/$target_file\"}" > "$ROOT_DIR/dist-packages/package-map.file.json"
        fi
      else
