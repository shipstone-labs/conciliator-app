...
contract IPDocV11 is ERC1155, AccessControl, ERC1155Pausable, ERC1155Burnable, ERC1155Supply, ERC1155URIStorage {
    ...
    mapping(address => mapping(bytes32 => uint256)) private _expirations;
    
    event ExpirationSet(address indexed account, bytes32 indexed tokenId, uint256 expirationTime);
     ...
     */
    function setExpiration(address account, bytes32 id, uint256 expirationTime) 
        public 
        onlyRole(EXPIRATION_MANAGER_ROLE) 
    {
        require(super.balanceOf(account, uint256(id)) > 0, "IPDocV11: Account has no balance to expire");
     ...
     */
    function setBatchExpiration(address account, bytes32[] memory ids, uint256[] memory expirationTimes)
        public
        onlyRole(EXPIRATION_MANAGER_ROLE)
    {
        require(ids.length == expirationTimes.length, "IPDocV11: ids and expirationTimes length mismatch");
        
        for (uint256 i = 0; i < ids.length; i++) {
            require(super.balanceOf(account, uint256(ids[i])) > 0, "IPDocV11: Account has no balance to expire");
     ...
     */
    function getExpiration(address account, bytes32 id) public view returns (uint256) {
     ...
     */
    function isExpired(address account, bytes32 id) public view returns (bool) {
    ...
    function balanceOf(address account, uint256 id) public view virtual override(ERC1155) returns (uint256) {
        ...
        bytes32 idAsBytes32 = bytes32(id);
        uint256 expiration = _expirations[account][idAsBytes32];
    ...
    {
        require(accounts.length == ids.length, "IPDocV11: accounts and ids length mismatch");
    ...
    function mint(address account, bytes32 id, uint256 amount, bytes memory data)
        public
    ...
    function mintBatch(address to, bytes32[] memory ids, uint256[] memory amounts, bytes memory data)
        public
        onlyRole(MINTER_ROLE)
    {
        uint256[] memory uintIds = new uint256[](ids.length);
        for (uint256 i = 0; i < ids.length; i++) {
            uintIds[i] = uint256(ids[i]);
        }
        
        _mintBatch(to, uintIds, amounts, data);
    }
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        safeTransferFrom(from, to, uint256(id), amount, data);
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        // Convert bytes32 ids to uint256 for ERC1155 compatibility
        uint256[] memory uintIds = new uint256[](ids.length);
        for (uint256 i = 0; i < ids.length; i++) {
            uintIds[i] = uint256(ids[i]);
        }
        ...
        // First transfer the tokens
        safeBatchTransferFrom(from, to, uintIds, amounts, data);
        ...
        override(ERC1155, ERC1155Pausable, ERC1155Supply)
    {
        ...
        if (from != address(0) && to != address(0)) {
            require(hasRole(MINTER_ROLE, _msgSender()), "IPDocV11: must have minter role to transfer");
        }
        ...
        if (from != address(0)) {
            for (uint256 i = 0; i < ids.length; i++) {
                ...
                if (values[i] > 0) {
                    // Convert uint256 to bytes32 for the expiration mapping
                    bytes32 idAsBytes32 = bytes32(ids[i]);
                    
                    // Check if the balance has expired
                    uint256 expiration = _expirations[from][idAsBytes32];
                    ...
                    if (expiration != 0 && expiration <= block.timestamp) {
                        ...
                        require(to == address(0), "IPDocV11: Cannot transfer expired tokens except to burn");
                    }
                }
            }
        }
            ...
            for (uint256 i = 0; i < ids.length; i++) {
                if (values[i] > 0) {
                    ...
                    if (super.balanceOf(from, ids[i]) == 0) {
                        delete _expirations[from][idAsBytes32];
                    }
                }
            }
        }
