...
export function useClientTracing() {
   ...
   */
  const traceComponent = useCallback((componentName: string, props: Record<string, any> = {}) => {
    useEffect(() => {
      ...
      const span = browserTracer.startSpan(`Component.${componentName}.Lifecycle`, {
        attributes,
      });
   ...
   */
  const traceAction = useCallback(<T extends any>(
    actionName: string, 
    fn: () => T | Promise<T>,
    attributes: Record<string, any> = {}
  ) => {
    const isPromise = typeof fn === 'function' && 
                     (fn.constructor.name === 'AsyncFunction' || 
                      fn.toString().includes('Promise'));
                      
    if (isPromise) {
      return tracePromise(`UserAction.${actionName}`, fn as () => Promise<T>, {
   ...
   */
  const traceNavigation = useCallback((path: string, metadata: Record<string, any> = {}) => {
    return traceFunction(`Navigation.${path}`, () => {
